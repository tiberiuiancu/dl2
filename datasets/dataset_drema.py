...
class DremaDataset(Dataset):
    ...
    def __init__(
        ...
        task, _ = taskvar[0]
        var = "all_variations"
        data_dir = self._root[0] / f"{task}/all_variations/episodes/"
        if not data_dir.is_dir():
            print(f"Can't find dataset folder {data_dir}")
        episodes = [(task, var, ep_path) for ep_path in sorted(data_dir.glob("episode*"))]
        """
        ...
        for task, eps in episodes_by_task.items():
            ...
            episodes_by_task[task] = sorted(
                eps, key=lambda t: int(re.search(r'\d+', str(t[2].stem)).group())
            )
            self._episodes += eps
